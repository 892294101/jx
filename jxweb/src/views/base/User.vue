  ...
  <el-card>
    ...
    <el-form size="mini" :inline="true" :model="queryParams">
      <el-form-item label="用户账号" prop="username">
        <el-input id="username" @mouseenter.native="userMouseenter" class="searchList" size="mini" placeholder="请输入用户账号" clearable v-model="queryParams.username"
                  @keyup.enter.native="handleQuery"/>
      </el-form-item>

      <el-form-item label="账号状态" prop="status">
        <el-select id="status" class="searchList" size="mini" placeholder="请选择账号状态"
                   v-model="queryParams.status" clearable filterable> <!-- filterable -->
          <el-option v-for="item in statusList" :key="item.value" :label="item.label" :value="item.value"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="开始时间" prop="beginTime">
        <el-date-picker size="mini" type="date" style="width: 135px" value-format="yyyy-MM-dd"
      ...
      <el-form-item>
        <el-button type="primary" icon="el-icon-search" size="mini" @click="handleQuery" :loading="ButtonStatusSets.QueryBut">搜索</el-button>
        <el-button type="primary" icon="el-icon-refresh" size="mini" @click="resetQuery" :loading="ButtonStatusSets.QueryBut">重置</el-button>
      </el-form-item>
      <!--操作按钮-->
      <el-row :gutter="10" class="mb8">
        <el-col :span="1.5">
          <el-button plain type="primary" icon="el-icon-plus" size="mini"
                     @click="addDialogVisible = true" v-authority="['base:admin:add']">新增
          </el-button>
        </el-col>
      </el-row>
      <!--列表-->
      <el-table size="mini" border stripe v-loading="Loading" :data="adminList" style="width: 100%;" :header-cell-style="{ background: '#eef1f6', color: '#606266' }">
        <el-table-column label="ID" prop="id" v-if="false"/>
        <el-table-column fixed label="用户账号" prop="username" min-width="120" width="auto"/>
        <el-table-column fixed label="用户昵称" prop="nickname" min-width="120" width="auto"/>
        <el-table-column label="用户手机" prop="phone" min-width="120" width="auto"/>
        <el-table-column label="用户邮箱" prop="email" min-width="170" width="auto"/>
        <!--        <el-table-column label="用户头像" prop="icon">
                  <template slot-scope="scope">
                    <el-avatar shape="circle" :src="scope.row.icon" size="small"></el-avatar>
                  </template>
                </el-table-column>-->
        ...
        <el-table-column label="部门/岗位" min-width="180" width="auto">
          <template slot-scope="scope">
            <div>{{ scope.row.deptName }} / {{ scope.row.postName }}</div>
          </template>
        </el-table-column>
        <el-table-column label="角色名称" prop="roleName" min-width="102" width="auto">
          <!--
          <template slot-scope="scope">
        ...
        <el-table-column label="更多操作" fixed="right" min-width="200" width="auto">
          <template slot-scope="scope">
            <el-button size="small" type="text" icon="el-icon-edit" @click="showEditAdminDialog(scope.row.id)" v-authority="['base:admin:edit']">编辑</el-button>
            <el-button size="small" type="text" icon="el-icon-delete" @click="handleAdminDelete(scope.row)" v-authority="['base:admin:delete']">删除</el-button>
            <el-button size="small" type="text" icon="el-icon-key" @click="handleResetPwd(scope.row)" v-authority="['base:admin:reset']">重置密码</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!--分页区域-->
    ...
    <el-dialog title="新增用户" :visible.sync="addDialogVisible" width="40%" @close="addDialogClosed">
      <el-form size="mini" :model="addForm" :rules="addFormRules" ref="addFormRefForm" label-width="80px">
        <el-row>
          <el-col :span="12">
          ...
          <el-col :span="12">
            <el-form-item label="归属部门" prop="deptId">
              ...
              <treeselect v-model="addForm.deptId" :options="deptList" :show-count="true" placeholder="请选择归属部门"/>
              -->
              <el-select placeholder="请选择归属部门" v-model="addForm.deptId" clearable>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="用户岗位" prop="postId">
              <el-select placeholder="请选择岗位" v-model="addForm.postId" clearable>
          ...
          <el-col :span="12">
            <el-form-item label="用户角色" prop="roleId">
              <el-select placeholder="请选择角色" v-model="addForm.roleId" clearable>
    ...
    <el-dialog title="修改用户" :visible.sync="editDialogVisible" width="40%" @close="editDialogClosed">
      <el-form size="mini" :model="adminInfo" :rules="editFormRules" ref="editFormRefForm" label-width="80px">
        <el-row>
          <el-col :span="12">
            <el-form-item id="nickname" label="用户昵称" prop="nickname">
          ...
          <el-col :span="12">
            <el-form-item label="归属部门" prop="deptId">
              <!--
              <el-form-item size="mini" label="归属部门" prop="deptId">
              <treeselect v-model="adminInfo.deptId" :options="deptList" :show-count="true" placeholder="请选择归属部门"/>
              -->
              <el-select placeholder="请选择归属部门" v-model="adminInfo.deptId" clearable>
                <el-option v-for="item in deptList" :key="item.id" :label="item.label" :value="item.id">
                </el-option>
              </el-select>
            </el-form-item>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="用户岗位" prop="postId">
              <el-select placeholder="请选择岗位" v-model="adminInfo.postId" clearable>
                <el-option v-for="item in postList" :key="item.id" :label="item.postName" :value="item.id">
                </el-option>
              </el-select>
            </el-form-item>
          ...
          <el-col :span="12">
            <el-form-item label="用户角色" prop="roleId">
              <el-select placeholder="请选择角色" v-model="adminInfo.roleId" clearable>
                <el-option v-for="item in roleList" :key="item.id" :label="item.roleName"
                           :value="item.id"></el-option>
              </el-select>
            </el-form-item>
...
export default {
  ...
  methods: {
    // 查询列表
    async getAdminList() {
      this.ButtonStatusSets.QueryBut = true
      this.Loading = true
      ...
      if (res.code !== 200) {
        ...
        this.Loading = false
        this.ButtonStatusSets.QueryBut = false
      }
    },
    // 搜索按钮操作
    handleQuery(e) {
